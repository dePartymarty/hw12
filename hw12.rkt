  ...
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'block)
       (list 'block-exp (map no-parser (cdr no-code)))) ;just replicated what was done in the else 'call-exp
...
(define run-block-expression
  (lambda (block env)
    (cond
      ((null? block) '())
      ((eq? (caar block) 'remember-exp) (run-block-expression (cdr block) (run-parsed-code (car block) env)))
      (else (cons (run-parsed-code (car block) env) (run-block-expression (cdr block) env))))))
  ...
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'block-exp)
       (run-block-expression (cadr parsed-no-code) env))
...
(define sample-no-code '(block (remember a 15) (remember b 17) (spit-out a) (spit-out b) (remember c (do-mathy-stuff + a b)) (spit-out c)))
